...
| sv_ref (mut: mutkind) (ph: option path) (view: views) (* reference
  to an owner at [phs]. We also record the reborrowed paths of ph. If
  ph is None, it means that it has been invalidated by the dynamic
  borrow check or it has been moved from. The dynamic borrow check
  should maintian an invariant that if some sv_ref is valid, then the
  path it points to must also valid. *)
  ...
  | pj :: l =>
      ...
      | proj_downcast fid1, sv_enum _ fid2 sv1 =>
          if ident_eq fid1 fid2 then
            ...
            Error nil
      | proj_deref, sv_ref mut (Some ph2) rebor =>
...
Fixpoint kill_paths_ref (kill: views) (sv: sval) : sval :=
  match sv with
  | sv_ref mut ph1 vs =>
      sv_ref mut ph1 (kill_paths kill vs)
  | sv_struct id svl =>
      sv_struct id (map (fun '(fid, fsv) => (fid, kill_paths_ref kill fsv)) svl)
  | sv_enum id fid sv1 =>
      sv_enum id fid (kill_paths_ref kill sv1)
  | sv_box sv1 =>
      sv_box (kill_paths_ref kill sv1)
  | _ => sv
  end.

Definition kill_paths_ref_svm (kill: views) (svm: sv_map) : sv_map :=
  PTree.map1 (fun '(r, ty, sv) => (r, ty, kill_paths_ref kill sv)) svm.
...
Fixpoint collect_sval_ref_paths (sv: sval) : list path :=
  ...
  (* How to prove that it must be Some ph? *)
  | sv_ref _ (Some ph) vs =>
      ph :: nil
...
Fixpoint recover_sval_ref_paths (process_views: views -> views) (l: list path) (sv: sval)  : res sval :=
  ...
  | sv_ref mut (Some ph) vs =>
      ...
      OK (sv_ref mut (Some ph1) (process_views vs))
  | sv_box sv1 =>
...
Fixpoint eval_pexpr (svm: sv_map) (pe: pexpr) : res (sval * sv_map) :=
  ...
  | Eref _ mut p _ =>
      ...
      OK (sv_ref mut (Some ph) vs, invalidate_conflict_ref_svm p Adeep svm)
  | _ => Error nil
  end.
...
Definition eval_expr (svm: sv_map) (e: expr) : res (sval * sv_map) :=
  ...
  | Emoveplace p _ =>
      ...
      (* We move from the value of this path. Should we move from
      (valid_owner p). We put sv_bot in it instead of using clear_sval
      like drop does because if we move from a mutable reference and
      we use clear_sval then the value of this reference remains in
      this location, so we cannot check the validity of a value
      correctly. *)
...
Fixpoint eval_exprlist (svm: sv_map) (al: list expr) (* (tyl: typelist) *) : res (list sval * sv_map) :=
  ...
  | a :: al1 =>
      ...
      (* do v1' <- sem_cast v1 (typeof a) ty; *)
      do (vl, svm2) <- eval_exprlist svm1 al1;
      OK (v1 :: vl, svm2)
  (* | _ => Error nil *)
  end.
...
Definition eval_assign (svm1: sv_map) (p: place) (e: expr) : res (sval * sv_map) :=
  do (sv, svm2) <- eval_expr svm1 e;
  do (ph, vs) <- get_owner_path_sv_map p svm2;
  let svm3 := invalidate_conflict_ref_svm p Ashallow svm2 in
  (* This property should be guaranteed by the correct insertion of
    drop. Since we cannot have no way to express this guarantee
    provided by RustIRgen/Drop Elaboration, we must encode it into the
    semantics. *)
  if check_path_is_dropped svm3 ph then
    (* Before overwrite the target location, we should first set it to
    sv_bot so that the original sv_ref in this location is removed and
    we can establish invariant before overwriting a new value *)
    do svm4 <- set_sval_map ph sv_bot svm3;
    (* To make the views at each reference precise *)    
    OK (sv, kill_paths_ref_svm vs svm4)
  else 
    Error nil.

Inductive step : state -> trace -> state -> Prop :=
| step_assign: forall f e (p: place) sv svm1 svm5 svm6 ph ns k
    (EVAL: eval_assign svm1 p e = OK (sv, svm5))
    (ASS: set_sval_map ph sv svm5 = OK svm6),
    step (State f (Sassign p e) k ns svm1) E0 (State f Sskip k ns svm6)
| step_assign_variant: forall f e (p: place) k svm1 svm5 svm6 sv co fid enum_id orgs ph fty ns
    (EVAL: eval_assign svm1 p e = OK (sv, svm5))
    ...
    (ASS: set_sval_map ph (sv_enum enum_id fid sv) svm5 = OK svm6),
    step (State f (Sassign_variant p enum_id fid e) k ns svm1) E0 (State f Sskip k ns svm5)
| step_box: forall f e (p: place) k ty svm1 svm5 svm6 sv ph ns
    (EVAL: eval_assign svm1 p e = OK (sv, svm5))
    (TYP: typeof_place p = Tbox ty)
    (* (CAST: sem_cast v (typeof e) ty = OK v1) *)
    (ASS: set_sval_map ph (sv_box sv) svm5 = OK svm6),
    step (State f (Sbox p e) k ns svm1) E0 (State f Sskip k ns svm5)
(** big-step drop semantics: just like a move operation *)
| step_drop: forall svm1 svm2 svm3 k f (p: place) ph ns vs
    (EVALP: get_owner_path_sv_map p svm1 = OK (ph, vs))
    (INVP: invalidate_conflict_ref_svm p Adeep svm1 = svm2)
    (DROP: clear_sval_map ph svm2 = OK svm3),
    step (State f (Sdrop p) k ns svm1) E0 (State f Sskip k ns svm3)
| step_storagelive: forall f k ns svm id,
    step (State f (Sstoragelive id) k ns svm) E0 (State f Sskip k ns svm)
| step_storagedead: forall f k ns svm1 svm2 id
    (* The insertion of drop should ensure that before storagedead,
    the resource of variable is released so that we can  *)
    (CKDROP: check_path_is_dropped svm1 (id, nil) = OK true)
    (* To maintain the invariant, we need to clear all the dead
    sv_ref, so we set it to sv_bot. *)
    (CLR: set_sval_map (id, nil) sv_bot svm1 = OK svm2),
    step (State f (Sstoragedead id) k ns svm1) E0 (State f Sskip k ns svm2)
| step_call: forall f ty al k tyargs fd cconv tyres p orgs org_rels fun_id svm1 svm2 args args1 inout_params ns phl
    ...
    (EVAL: eval_exprlist svm1 al = OK (args, svm2))
    (NOT_DROP: function_not_drop_glue fd)
    (* Collect the footprint that is passed via reference *)
    (REF_OUT: generate_call_parameters ge svm2 (combine (type_list_of_typelist tyargs) args) = OK (args1, inout_params, phl)),
    step (State f (Scall p (Eglobal fun_id ty) al) k ns svm1) E0 (Callstate fun_id args1 inout_params (Kcall p f phl ns svm2 k))
| step_internal_function: forall fun_id vargs inout_params k svm1 f ns
    (FINDF: ge.(genv_defmap) ! fun_id = Some (Gfun (Internal f)))
    (NORMAL: f.(fn_drop_glue) = None)
    (ENTRY: function_entry f vargs inout_params = OK (ns, svm1)),
    step (Callstate fun_id vargs inout_params k) E0 (State f f.(fn_body) k ns svm1)

| step_return_1: forall p v v1 v2 svm1 svm2 f k ck ns out_params
    (CONT: call_cont k = Some ck)
    (EVAL: eval_pexpr svm1 (Eplace p (typeof_place p)) = OK (sv, svm2))
    (* (CAST: sem_cast v (typeof_place p) f.(fn_return) = OK v1) *)
    (** Rename the external footprint to match the use of the names passed
    in this function *)
    (* check that all local variables/paramters have been dropped *)
    (* perform shallow write on all the local variables/parameters *)
    (* kill all paths related to local variables/parameters *)
    (** How to know in advance that all the sval in out parameters
    does not contain sv_bot at the reference place? We must do a
    check? Which should be related to check_storagedead_list? *)
